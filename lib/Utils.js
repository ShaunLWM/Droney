let self = (module.exports = {
	...
	parseTorrent({ type, hash, data }) {
		function parseState(state, size, completed) {
			if (state === 0) return "closed";
			if (state === 1 && size === completed) return "seeding";
			if (state === 1 && size !== completed) return "downloading";
			return "unknown";
		}
		...
		switch (type) {
			case "rutorrent":
				return {
					...
					name: data[4],
					state: Number(data[3]),
					status: parseState(Number(data[3]), Number(data[5]), Number(data[8])),
					dir: data[25],
					size: Number(data[5]),
					left: Number(data[19]),
					completed: Number(data[8]),
					ratio: Number(data[10]) / 1000,
					up_rate: Number(data[11]), // /KiB
					down_rate: Number(data[12]),
					uploaded: Number(data[9]),
					peers_connected: Number(data[17]),
					seeders: Number(data[18]),
					leechers: Number(data[17]) - Number(data[18])
				};
		}
	}
});
